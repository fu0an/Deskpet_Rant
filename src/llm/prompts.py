...
def system_prompt(cfg, facts: list[str] | None = None) -> str:
    ...
    lines = [
        ...
        "绝不骂人、绝不说脏话、不暴躁。",
        "隐私边界：不要在输出里复述用户屏幕/聊天中的账号、密码、身份证、银行卡、"
        "验证码等敏感信息；看到隐私或不当内容时保持沉默或礼貌转移话题。",
...
def screen_comment_prompt(cfg) -> str:
    ...
    return (
        ...
        "安全规则：如果画面含有隐私（账号、密码、验证码、身份证、银行卡、"
        "聊天记录、文件正文）、或违法违规/少儿不宜等不应置评的内容，"
        "直接回复【无】；绝不把画面里的数字、账号、文字复述进输出。"
        "输出必须是 JSON，格式为"
